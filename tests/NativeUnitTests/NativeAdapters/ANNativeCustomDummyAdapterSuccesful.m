...
#import "ANNativeCustomDummyAdapterSuccesful.h"

@interface ANNativeCustomDummyAdapterSuccesful ()

@end

@implementation ANNativeCustomDummyAdapterSuccesful
...
- (void)requestNativeAdWithServerParameter:(NSString *)parameterString
    ...
    ANNativeMediatedAdResponse *response = [[ANNativeMediatedAdResponse alloc] initWithCustomAdapter:self
                                                                                         networkCode:ANNativeAdNetworkCodeCustom];
    response.title = @"ANNativeAdapterSuccesful";
    response.body = @"DUMMY BODY";
...
- (void)registerViewForImpressionTrackingAndClickHandling:(UIView *)view
    ...
    // JUST logging impression when view is registered for tracking simulating adDidLogImpression callback.
